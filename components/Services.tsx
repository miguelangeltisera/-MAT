...
const ServiceCard: React.FC<{ service: Service; index: number }> = ({ service, index }) => {
  ...
  return (
    <div 
      className="bg-white/40 backdrop-blur-sm border border-green-500/30 p-6 rounded-lg transition-all duration-300 hover:border-green-400 hover:shadow-[0_0_25px_rgba(16,185,129,0.4)] hover:-translate-y-2 animate-fade-in-up"
      style={{ animationDelay: `${index * 150}ms` }}
    >
      <div className="flex items-center justify-center w-12 h-12 bg-green-200/50 border border-green-500/50 rounded-lg mb-4">
        <Icon className="w-6 h-6 text-green-600" />
      </div>
      <h3 className="text-xl font-bold text-green-900 mb-2">{service.title}</h3>
      <p className="text-green-800/90">{service.description}</p>
    </div>
  );
};
    ...
    <div className="py-16 md:py-24 px-4">
      <div className="container mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-green-900 tracking-wide">Nuestros Dominios Creativos</h2>
          <div className="mt-4 w-24 h-1 mx-auto bg-green-500 rounded-full"></div>
        </div>
